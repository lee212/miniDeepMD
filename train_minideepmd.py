...
def setup_wandb(
    cfg: Point3dAAEConfig, model: torch.nn.Module, comm_rank: int
    ) -> Optional[wandb.sdk.lib.preinit.PreInitObject]:#wandb.config]:
    # Setup wandb
    wandb_config = None
...
def get_h5_training_file(cfg: Point3dAAEConfig) -> Tuple[Path, List[str]]:
    ...
    virtual_h5_path, h5_files = get_virtual_h5_file(
        ...
        last_n=0,#cfg.last_n_h5_files,
        k_random_old=0,#cfg.k_random_old_h5_files,
...
def get_init_weights(cfg: Point3dAAEConfig) -> Optional[str]:
...
def main(
    ...
    model_hparams = AAE3dHyperparams(
        ...
        encoder_kernel_sizes=cfg.encoder_kernels,
        latent_dim=cfg.latent_dim,
        encoder_relu_slope=cfg.encoder_relu_slope,
        generator_relu_slope=cfg.decoder_relu_slope,
        discriminator_relu_slope=cfg.discriminator_relu_slope,
        use_encoder_bias=cfg.encoder_bias,
        use_generator_bias=cfg.decoder_bias,
        use_discriminator_bias=cfg.discriminator_bias,
    ...
    optimizer_hparams = OptimizerHyperparams(
        name=cfg.ae_optimizer.name, hparams=cfg.ae_optimizer.hparams
    )
    ...
    wandb_config = None #= setup_wandb(cfg, aae.model, comm_rank)
